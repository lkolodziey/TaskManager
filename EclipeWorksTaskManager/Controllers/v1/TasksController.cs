...
public class TasksController(ITaskService service) : ControllerBase
{
    ...
    public async Task<IActionResult> GetByProjectIdAsync(int projectId)
    {
        try
        {
            ...
            return result.Count > 0 ? Ok(new { Success = true, Data = result }) : BadRequest(new { Success = false });
        }
        catch (Exception e)
        {
            return Problem(
    ...
    [HttpPost]
    public async Task<IActionResult> PostAsync([FromBody] TaskRequest model)
    {
        try
        {
    ...
    [HttpPut]
    public async Task<IActionResult> PutAsync([FromQuery] int id, [FromBody] TaskRequest model)
    {
        try
        {
